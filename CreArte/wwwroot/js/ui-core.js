...
// ======================================================
    ...
    // <-- NUEVO: NIVEL — enciende el icono si hay un valor aplicado
        setActive('[data-pop="popNivel"]', document.getElementById('ua_val_nivel')?.value);
})();
    ...
    // <-- NUEVO: NIVEL — limpiar parámetro previo "Nivel" si existiera
    f.querySelectorAll(`[name="Nivel"]`).forEach(nod => nod.remove());
    ...
    const i = document.getElementById('fArea'); if (i) i.value = '';
        ...
        function uaApplyFecha() {
            submitFilters({
                ...
                // <-- NUEVO: NIVEL — preservar el valor de nivel al aplicar FECHA
                Nivel: document.getElementById('fNivel')?.value || document.getElementById('ua_val_nivel')?.value,
                FechaInicio: document.getElementById('fIni')?.value,
                FechaFin: document.getElementById('fFin')?.value,
                Estado: document.getElementById('fEstado')?.value
            });
    ...
    const i = document.getElementById('fIni'), f = document.getElementById('fFin');
        ...
        function uaApplyEstado() {
            submitFilters({
                Area: document.getElementById('fArea')?.value,
                // <-- NUEVO: NIVEL — preservar el valor de nivel al aplicar ESTADO
                Nivel: document.getElementById('fNivel')?.value || document.getElementById('ua_val_nivel')?.value,
                FechaInicio: document.getElementById('fIni')?.value,
                FechaFin: document.getElementById('fFin')?.value,
                Estado: document.getElementById('fEstado')?.value
    ...
    const txt = (document.getElementById('fNivel')?.value || '').trim();
        const opt = (document.querySelector('input[name="optN"]:checked') || { }).value || 'all';
        ...
        submitFilters({
            Nivel: val, // clave correcta para el backend
        // preserva los demás filtros tal como están
        Area:        document.getElementById('fArea')?.value,
        FechaInicio: document.getElementById('fIni')?.value,
        FechaFin:    document.getElementById('fFin')?.value,
        Estado:      document.getElementById('fEstado')?.value
        ...
        function uaClearNivel() {
    const i = document.getElementById('fNivel'); if (i) i.value = '';
        submitFilters({
            Nivel: '',
        Area:        document.getElementById('fArea')?.value,
        FechaInicio: document.getElementById('fIni')?.value,
...
/* <-- NUEVO: Enter en el input de NIVEL = aplicar */
        document.addEventListener('keydown', function (e) {
    if (e.key === 'Enter' && e.target && e.target.id === 'fNivel') {e.preventDefault(); uaApplyNivel(); }
});
